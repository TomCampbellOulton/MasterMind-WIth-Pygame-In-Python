    ...
    while leaderPage:
        for event in pygame.event.get():
            ...
            for b in buttons:
                ...
                if buttonPressed == "":
                    print("Leaderboards :D")
                elif buttonPressed == "MainMenu":
...
def makeButtons(oldx, oldy, numOfColumns, colours):
    ...
    saveGameButton = Button("Save Game", (width-320, 100), font=30, bg="Red", saveGameButton=True)
...
def runGame(n, numberOfColumns, width, height, listOfColours, allPegs = [], allUsersAnswers = [], numberOfTurnsUsed = 0, board=[]):
    ...
    while gameOn:
        # Start the game
        buttons, oldx, oldy, numberOfRowsLeft, gameOn =  displayGame(buttons, listOfColours, oldx, oldy, numberOfColumns, board, numberOfRowsLeft, gameOn, board, width, height, allPegs, allUsersAnswers)
    ...
    updateLeaderboards(len(listOfColours), numberOfColumns, n-numberOfRowsLeft)
    return won, n-numberOfRowsLeft

def displayGame(buttons, listOfColours, oldx, oldy, numberOfColumns, correctAnswer, numberOfRowsLeft, gameOn, board, screenWidth, screenHeight, allPegs = [], allUsersAnswers = []):
    ...
    for event in pygame.event.get():
        ...
        if createNewRow:
            ...
            saveGameButton = Button("Save Game", (screenWidth-320, 100), font=30, bg="Red", saveGameButton = True)
            buttons.append(mainMenuButton)
            buttons.append(saveGameButton)
            mainMenuButton.show()
            saveGameButton.show()
    ...
    while running:
        for event in pygame.event.get():
            ...
            for b in buttons:
                ...
                elif buttonPressed == "SaveGame":
                    print("Save Game :D")
                elif buttonPressed == "LoadGame":
                    print("Load Game :D")
                    loadGame(screenWidth)
                #else:
                 #   print("You let me walk this road alone.")
...
def displayLoadedGame(numberOfColumns, numberOfRows, numberOfRowsRemaining, colouredPegs, usersChoices, screenWidth, listOfColours, board,width, height):
   ...
   # print(f"Coloured pegs = {colouredPegs}")
...
def helpPage(numberOfColumns, numberOfRows, listOfColours, screenWidth, screenHeight):
    ...
    pygame.font.init()
    myFont = pygame.font.SysFont("Comic Sans MS", 30)
    textSurface = myFont.render(f"Help Is Needed D:", False, (100,100,100))
    size = textSurface.get_size()
    surface = pygame.Surface(size)
    surface.blit(textSurface, (0, 0))
    screen.blit(textSurface, (50,10))
    pygame.display.update()
    """
    writeText("Help Page:",450,60, fontSize=50)
    writeText("Scoring:",150,160, fontSize=40)
    writeText("Score = (Number of colours chosen X number of columns) ^ (1 ÷ number of turns taken)  X 1,000",100,200, fontSize=30, fontColour="Grey")

    writeText("Colours:",150,260, fontSize=40)
    writeText("You can use between 1 and 14 colours, depending on the difficulty you desire.",100,300, fontSize=30, fontColour="Grey")
    writeText("These colours are: yellow, red, orange, blue, purple, green, crimson, coral, cyan,",100,340, fontSize=30, fontColour="Grey")
    writeText("                   dark blue, dark green, dark salmon, deep pink and pink.",100,380, fontSize=30, fontColour="Grey")

    writeText("Saving:",150,480, fontSize=40)
    writeText("You can only save 1 game. Once you save a new game, the old game that you saved is ",100,520, fontSize=30, fontColour="Grey")
    writeText("overridden. When you choose 'Load Game', the newest save will be loaded automatically.",100,560, fontSize=30, fontColour="Grey")

    writeText("Leaderboards:",150,660, fontSize=40)
    writeText("The order on the leaderboards are determined by the higher the score, the higher tha placement.",100,700, fontSize=30, fontColour="Grey")
    writeText("if you were to tie with a previous attempt, the older attempt will be ranked higher. All games played",100,740, fontSize=30, fontColour="Grey")
    writeText("are recorded.",100,780, fontSize=30, fontColour="Grey")
